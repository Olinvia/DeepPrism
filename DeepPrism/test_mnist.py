...
for i in tqdm(range(120)):
    ...
    st = time.time()
    proven, iteration = r2model.certify(input_dp, y, verbose=args.verbose)
    total_iteration = total_iteration + iteration
    if proven:
        success_iteration = success_iteration + iteration
    ed = time.time()
    running_time += ed - st
...
with open(res_name, "a") as f:
    f.write(f"{bound_method},{eps},{proven_dp/correct*100},{running_time/correct},{total_iteration/correct},{success_iteration/proven_dp}\n")
