...
class LSTMCell(nn.LSTM, DPBackSubstitution):
    ...
    def set_lambda(self, device=torch.device("cpu")):
        self.lmb = (
            torch.Tensor(2, self.num_layers, 3, self.hidden_size, 10)
            .to(device)
            .uniform_(-1, 1)
        )
    ...
    def forward(self, x_t_dp):
        ...
        sf = nn.Softmax(dim=-1)
        split_type = [0,11,12,13,21,22,23,31,32,33]
        ...
        for i in range(self.num_layers):
            ...
            if self.prev_cell is not None:
                if not self.initialised:
                    coeff = torch.zeros(6, hsize, 10 if self.method == "opt" else 1).to(
                        dev
                    )
            ...
            if not self.initialised:
                coeff = torch.zeros(6, hsize, 10 if self.method == "opt" else 1).to(dev)
                for d in range(hsize):
                    if self.method == "opt":
                        for p in range(10):
                            if p == 0:
