...
def LUB_split(lx, ux, ly, uy, tanh=True, split_type=0, n_samples=200):
    ...
    # (lx,ly)   (ux,ly)     (ux,uy)
    if split_type == 11:
        zu = Au*(0.75*lx+0.25*ux)+Bu*(0.75*ly+0.25*uy)+Cu
        zl = Al*(0.75*lx+0.25*ux)+Bl*(0.75*ly+0.25*uy)+Cl
        height = zu - zl
        obj = 0.25 * (ux-lx) * (uy-ly) * height
    # (lx,ly)   (lx,uy)     (ux,uy)
    elif split_type == 12:
        zu = Au*(0.75*lx+0.25*ux)+Bu*(0.25*ly+0.75*uy)+Cu
        zl = Al*(0.75*lx+0.25*ux)+Bl*(0.25*ly+0.75*uy)+Cl
        height = zu - zl
        obj = 0.25 * (ux - lx) * (uy - ly) * height
    # (lx,ly)   (lx,uy)     (ux,ly)
    elif split_type == 21:
        zu = Au*(0.25*lx+0.75*ux)+Bu*(0.75*ly+0.25*uy)+Cu
        zl = Al*(0.25*lx+0.75*ux)+Bl*(0.75*ly+0.25*uy)+Cl
        height = zu - zl
        obj = 0.25 * (ux - lx) * (uy - ly) * height
    # (lx,uy)   (ux,ly)     (ux,uy)
    elif split_type == 22:
        zu = Au*(0.25*lx+0.75*ux)+Bu*(0.25*ly+0.75*uy)+Cu
        zl = Al*(0.25*lx+0.75*ux)+Bl*(0.25*ly+0.75*uy)+Cl
        height = zu - zl
        obj = 0.25 * (ux - lx) * (uy - ly) * height
    model.setObjective(obj, GRB.MINIMIZE)
