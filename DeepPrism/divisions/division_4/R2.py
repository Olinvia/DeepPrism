...
from relaxation import LUB_split
...
class LSTMCell(nn.LSTM, DPBackSubstitution):
    ...
    def set_lambda(self, device=torch.device("cpu")):
        self.lmb = (
            torch.Tensor(2, self.num_layers, 3, self.hidden_size, 3)
            .to(device)
            .uniform_(-1, 1)
        )
    ...
    def forward(self, x_t_dp):
        ...
        for i in range(self.num_layers):
            ...
            if self.prev_cell is not None:
                if not self.initialised:
                    coeff = torch.zeros(6, hsize, 3 if self.method == "opt" else 1).to(
                        dev
                    )
            ...
            if not self.initialised:
                coeff = torch.zeros(6, hsize, 3 if self.method == "opt" else 1).to(dev)
                for d in range(hsize):
                    if self.method == "opt":
                        for p in range(3):
                            if p == 0:
