...
def L2UB(lx, ux, ly, uy, tanh=True, n_samples=104, n_seed=242):
    bndX = np.array([lx, lx, ux, ux])
    bndY = np.array([ly, uy, ly, uy])
...
def LB_split(lx, ux, ly, uy, tanh=True, split_type=0, n_samples=200):
    ...
    if split_type == 11:
        ...
        obj += f((lx+ux)/2, uy, tanh) - Al * (lx+ux)/2 - Bl * uy - Cl
        obj += f((lx+ux)/2, ly, tanh) - Al * (lx+ux)/2 - Bl * ly - Cl
    elif split_type == 12:
...
def UB_split(lx, ux, ly, uy, tanh=True, split_type=0, n_samples=200):
    ...
    if split_type == 11:
        ...
        obj -= f((lx+ux)/2, uy, tanh) - Au * (lx+ux)/2 - Bu * uy - Cu
        obj -= f((lx+ux)/2, ly, tanh) - Au * (lx+ux)/2 - Bu * ly - Cu
    elif split_type == 12:
...
def LUB_split(lx, ux, ly, uy, tanh=True, split_type=0, n_samples=200):
    ...
    # (lx,ly)   (ux,ly)     (ux,uy)
    if split_type == 11:
        zu = Au*(0.5*lx+0.5*ux)+Bu*(0.75*ly+0.25*uy)+Cu
        zl = Al*(0.5*lx+0.5*ux)+Bl*(0.75*ly+0.25*uy)+Cl
        height = zu - zl
        obj = 0.5 * (ux-lx) * (uy-ly) * height
    # (lx,ly)   (lx,uy)     (ux,uy)
    elif split_type == 12:
        zu = Au*(0.5*lx+0.5*ux)+Bu*(0.25*ly+0.75*uy)+Cu
        zl = Al*(0.5*lx+0.5*ux)+Bl*(0.25*ly+0.75*uy)+Cl
        height = zu - zl
        obj = 0.5 * (ux - lx) * (uy - ly) * height
    model.setObjective(obj, GRB.MINIMIZE)
